...
//	String sql = "";
//		
//		try (Connection conn = DbConnection.getConnection()) { //--> 1. GETS CONNECTION &
//			startTransaction(conn);							//-----> 2. STARTS TRANSACTION
//			
//
...
//				}
//				stmt.executeBatch();
//				commitTransaction(conn); //----> 5. OR BE COMMITED (SUCCESSFUL)
//				return something;
//			} catch (Exception e) {
//				rollbackTransaction(conn); //--> 4. IF TRANSACTION START, NEEDS TO ROLLBACK (FAIL)
//				throw new DbException(e);
//			}
//		} catch (SQLException e) { //----------> 3. IF FAILS, THROWS EXCEPTION
//			throw new DbException(e);
//	}
...
//4) Modify Project	
	...
	boolean modified = stmt.executeUpdate() == 1; //"==1" means "if one row was updated"
		commitTransaction(conn);
